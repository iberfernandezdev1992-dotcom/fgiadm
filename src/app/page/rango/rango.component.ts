...
import { FormRangoComponent } from 'src/app/modal/form-rango/form-rango.component';
...
export class RangoComponent implements OnInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this._http.get(`rango`, params).subscribe({
      next: (responser: any) => {
  ...
  create(): void {
    // console.log('modal');
    const dialogRef = this.dialog.open(FormRangoComponent, {
      // width: '100vw',
      disableClose: true,
      data: { data: '' },
    });
  ...
  delete(data: any): void {
    const dialogRef = this.dialog.open(ConfirmacionComponent, {
      width: '250px',
      data: {
        title: '¿Estás seguro de que deseas eliminar a este Rango?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this._http.delete(`rango/${data.id}`).subscribe({
          next: (response) => {
