...
import { FormCompraActividadComponent } from 'src/app/modal/form-compra-actividad/form-compra-actividad.component';
import { FormCompraComponent } from 'src/app/modal/form-compra/form-compra.component';
import { FormGastoComponent } from 'src/app/modal/form-gasto/form-gasto.component';
...
import { VerInscripcionCertificacionDetalleComponent } from 'src/app/modal/ver-inscripcion-certificacion-detalle/ver-inscripcion-certificacion-detalle.component';
import { VerInscripcionTallerDetalleComponent } from 'src/app/modal/ver-inscripcion-taller-detalle/ver-inscripcion-taller-detalle.component';
...
export class ActividadShowComponent implements OnInit {
  ...
  loadData(): void {
    this.spinner.show();
    this._http.get(`actividads/${this.dataId}`).subscribe({
      next: (responser: any) => {
  ...
  createG(data = ''): void {
    ...
    const dialogRef = this.dialog.open(FormGastoComponent, {
      // width: '100vw',
      disableClose: true,
      data: { data, evento: this.data, url: 'actividadGasto' },
    });
  ...
  createC(data = ''): void {
    // console.log('modal');
    const dialogRef = this.dialog.open(FormCompraActividadComponent, {
      // width: '100vw',
      disableClose: true,
  ...
  message(m: string): void {
    ...
    verventa(data = '') {
      ...
      // console.log('modal');
      const dialogRef = this.dialog.open(VerInscripcionTallerDetalleComponent, {
        // width: '100vw',
        disableClose: true,
        data: { data },
      });
