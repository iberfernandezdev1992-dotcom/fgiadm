...
import { FormInstructorComponent } from 'src/app/modal/form-instructor/form-instructor.component';
...
export class InstructorComponent implements OnInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this._http.get(`instructor`, params).subscribe({
      next: (responser: any) => {
  ...
  create(): void {
    // console.log('modal');
    const dialogRef = this.dialog.open(FormInstructorComponent, {
      // width: '100vw',
      disableClose: true,
      data: { data: '' },
    });
  ...
  delete(data: any): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this._http.delete(`instructor/${data.id}`).subscribe({
          next: (response) => {
