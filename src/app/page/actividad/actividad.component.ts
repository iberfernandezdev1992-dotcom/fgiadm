...
export class ActividadComponent implements OnInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this._http.get(`actividads`, params).subscribe({
      next: (responser: any) => {
  ...
  create(): void {
    // console.log('modal');
    const dialogRef = this.dialog.open(FormActividadComponent, {
      // width: '100vw',
      disableClose: true,
      data: { data: '' },
    });
  ...
  delete(data: any): void {
    const dialogRef = this.dialog.open(ConfirmacionComponent, {
      width: '250px',
      data: {
        title: '¿Estás seguro de que deseas eliminar a esta Actividad?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this._http.delete(`actividads/${data.id}`).subscribe({
          next: (response) => {
  ...
  onSelected(event: MatAutocompleteSelectedEvent): void {
    ...
    if (event.option.value) {
      this.filtertable.certificacion_id = event.option.value.id;
    }
    this.loadData();
  }
  ...
  clearSelection(): void {
    ...
    this.filtertable.certificacion_id = '';
    this.filteredData = [];
    this.paginator.pageIndex = 0;
    this.loadData();
  }
