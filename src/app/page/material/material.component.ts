...
import { FormMaterialComponent } from 'src/app/modal/form-material/form-material.component';
...
export class MaterialComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // this.nombrecertificado = this.route.snapshot.paramMap.get('nombre');
    // if (this.dataId) {
    //   this.filtertable.volumen_id = this.dataId;
    // }
    const navigation = this.router.getCurrentNavigation();
    this.datacertificado = navigation?.extras.state?.['volumendata'];
    console.log(this.datacertificado);

    if (this.datacertificado) {
      this.filtertable.volumen_id = this.datacertificado.id;
    }
  }
  ngOnDestroy(): void {}
  ngOnInit(): void {
  ...
  loadData(): void {
    ...
    this._http.get(`materials`, params).subscribe({
      next: (responser: any) => {
  ...
  create(): void {
    ...
    const dialogRef = this.dialog.open(FormMaterialComponent, {
      // width: '100vw',
      disableClose: true,
      data: { volumenData: this.datacertificado, data:'' },
    });
  ...
  delete(data: any): void {
    const dialogRef = this.dialog.open(ConfirmacionComponent, {
      width: '250px',
      data: {
        title: '¿Estás seguro de que deseas eliminar a este Material?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this._http.delete(`materials/${data.id}`).subscribe({
          next: (response) => {
