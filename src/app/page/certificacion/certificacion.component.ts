...
import { FormCertificacionComponent } from 'src/app/modal/form-certificacion/form-certificacion.component';
...
export class CertificacionComponent implements OnInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this._http.get(`certificaciones`, params).subscribe({
      next: (responser: any) => {
  ...
  create(): void {
    // console.log('modal');
    const dialogRef = this.dialog.open(FormCertificacionComponent, {
      // width: '100vw',
      disableClose: true,
      data: { data: '' },
    });
  ...
  delete(data: any): void {
    const dialogRef = this.dialog.open(ConfirmacionComponent, {
      width: '250px',
      data: {
        title: '¿Estás seguro de que deseas eliminar a esta Programa?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this._http.delete(`certificaciones/${data.id}`).subscribe({
          next: (response) => {
