...
import { FormPaisComponent } from 'src/app/modal/form-pais/form-pais.component';
...
export class PaisComponent implements OnInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this._http.get(`pais`, params).subscribe({
      next: (responser: any) => {
  ...
  create(): void {
    // console.log('modal');
    const dialogRef = this.dialog.open(FormPaisComponent, {
      // width: '100vw',
      disableClose: true,
      data: { data: '' },
    });
  ...
  delete(data: any): void {
    const dialogRef = this.dialog.open(ConfirmacionComponent, {
      width: '250px',
      data: {
        title: '¿Estás seguro de que deseas eliminar a este Pais?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this._http.delete(`pais/${data.id}`).subscribe({
          next: (response) => {
