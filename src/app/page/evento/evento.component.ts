...
import { FormEventoComponent } from 'src/app/modal/form-evento/form-evento.component';
...
export class EventoComponent implements OnInit, OnDestroy {
    ...
    this.searchDataCtrl.valueChanges
      .pipe(
        ...
        switchMap((value: any) => {
          ...
          return this._http.get('eventos', params).pipe(
            finalize(() => {
  ...
  loadData(): void {
    ...
    this._http.get(`eventos`, params).subscribe({
      next: (responser: any) => {
  ...
  delete(data: any): void {
    const dialogRef = this.dialog.open(ConfirmacionComponent, {
      width: '250px',
      data: {
        title: '¿Estás seguro de que deseas eliminar a este Evento?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this._http.delete(`eventos/${data.id}`).subscribe({
          next: (response) => {
  ...
  onSelected(event: MatAutocompleteSelectedEvent): void {
    ...
    if (event.option.value) {
      this.filtertable.ciudad_id = event.option.value.id;
      this.filtertable.volumen_id = event.option.value.id;
    }
    this.loadData();
  }
  ...
  clearSelection(): void {
    ...
    this.filtertable.ciudad_id = '';
    this.filtertable.volumen_id = '';
    this.filteredData = [];
    this.paginator.pageIndex = 0;
    this.loadData();
  }
