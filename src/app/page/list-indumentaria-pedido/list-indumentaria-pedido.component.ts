...
import { FormIndumentariaComponent } from 'src/app/modal/form-indumentaria/form-indumentaria.component';
import { VerIndumentariaPedidoComponent } from 'src/app/modal/ver-indumentaria-pedido/ver-indumentaria-pedido.component';

@Component({
  selector: 'app-list-indumentaria-pedido',
  templateUrl: './list-indumentaria-pedido.component.html',
  styleUrls: ['./list-indumentaria-pedido.component.scss'],
})
export class ListIndumentariaPedidoComponent {
    ...
    this.searchDataCtrl.valueChanges
      .pipe(
        ...
        switchMap((value: any) => {
          ...
          return this._http.get('certificaciones', params).pipe(
            finalize(() => {
  ...
  loadData(): void {
    ...
    this._http.get(`ventas`, params).subscribe({
      next: (responser: any) => {
  ...
  create(): void {
    // console.log('modal');
    const dialogRef = this.dialog.open(FormIndumentariaComponent, {
      // width: '100vw',
      disableClose: true,
      data: { data: '' },
    });
  ...
  delete(data: any): void {
    const dialogRef = this.dialog.open(ConfirmacionComponent, {
      width: '250px',
      data: {
        title: '¿Estás seguro de que deseas eliminar a esta Indumentaria?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this._http.delete(`indumentarias/${data.id}`).subscribe({
          next: (response) => {
  ...
  clearSelection(): void {
    ...
    // this.filtertable.certificacion_if = '';
    this.filteredData = [];
    this.paginator.pageIndex = 0;
    this.loadData();
  }
