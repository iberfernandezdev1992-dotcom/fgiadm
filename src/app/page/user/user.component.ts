...
import { FormUserComponent } from 'src/app/modal/form-user/form-user.component';
...
export class UserComponent implements OnInit, OnDestroy {
  ...
  delete(data: any): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.spinner.show();
        this.http.delete(`users/${data.id}`).subscribe({
          next: (response) => {
  ...
  //     estado,
  //   };
  //   this.http.put(`users/${item.id}`, data).subscribe({
  //     next: (responser: any) => {
  //       // console.log(responser);
  //       item.estado = estado;
