...
export class GastoComponent implements OnInit, OnDestroy {
    ...
    this.searchDataCtrl.valueChanges
      .pipe(
        ...
        switchMap((value: any) => {
          ...
          return this._http.get('gastos', params).pipe(
            finalize(() => {
  ...
  loadData(): void {
    ...
    this._http.get(`gastos`, params).subscribe({
      next: (responser: any) => {
  ...
  onSelected(event: MatAutocompleteSelectedEvent): void {
    ...
    if (event.option.value) {
      this.filtertable.evento_id = event.option.value.id;
    }
    this.loadData();
  }
