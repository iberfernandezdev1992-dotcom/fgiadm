...
import { FormGaleriaComponent } from 'src/app/modal/form-galeria/form-galeria.component';
...
export class GaleriaComponent implements OnInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this._http.get(`galerias`, params).subscribe({
      next: (responser: any) => {
  ...
  create(): void {
    // console.log('modal');
    const dialogRef = this.dialog.open(FormGaleriaComponent, {
      // width: '100vw',
      disableClose: true,
      data: { data: '' },
    });
  ...
  delete(data: any): void {
    const dialogRef = this.dialog.open(ConfirmacionComponent, {
      width: '250px',
      data: {
        title: '¿Estás seguro de que deseas eliminar a esta Galeria?',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  onSelected(event: MatAutocompleteSelectedEvent): void {
    ...
    if (event.option.value) {
      this.filtertable.indumentaria_id = event.option.value.id;
    }
    this.loadData();
  }
  ...
  clearSelection(): void {
    ...
    this.filtertable.indumentaria_id = '';
    this.filteredData = [];
    this.paginator.pageIndex = 0;
    this.loadData();
  }
