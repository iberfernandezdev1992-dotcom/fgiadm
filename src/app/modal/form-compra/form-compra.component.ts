...
export class FormCompraComponent implements AfterViewInit, OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<FormCompraComponent>,
  ...
  createForm(): FormGroup {
    return this._formBuilder.group({
      id: [this.data.id],
      volumen: [
        this.evento ? this.evento.volumen_id : this.data.volumen,
      ...
      calificacion_practico: [this.data.calificacion_practico],
      calificacion_teorico: [this.data.calificacion_teorico],
      calificacion_total: [this.data.calificacion_total],
      forma_pago: [
  ...
  guardar() {
    ...
    if (this.evento) {
        formData.append('evento_id', this.evento.id);
        formData.append('volumen_id', this.evento.volumen_id);
    } else {
        formData.append('volumen_id', data1.volumen ? data1.volumen.id : '');
    }
    ...
    formData.append('calificacion_practico', calificacionPractico !== null ? calificacionPractico.toString() : '');
    formData.append('calificacion_teorico', calificacionTeorico !== null ? calificacionTeorico.toString() : '');
    formData.append('calificacion_total', data1.calificacion_total.toString());
    ...
    this.spinner.show();
    if (!data1.id) {
        this._http.post('compras', formData).subscribe({
            next: (response) => {
